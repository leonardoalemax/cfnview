...
export default function UserHeader({ info }: { info: SF6FighterBannerInfo }) {
	...
	const totalPP =
		playPoint.fighting_ground + playPoint.world_tour + playPoint.battle_hub;
	const fgPct =
		totalPP > 0
			? Math.round((playPoint.fighting_ground / totalPP) * 100)
			: 0;
	const wtPct =
		totalPP > 0 ? Math.round((playPoint.world_tour / totalPP) * 100) : 0;
	const bhPct =
		totalPP > 0 ? Math.round((playPoint.battle_hub / totalPP) * 100) : 0;
				...
				<div className='flex flex-col gap-3 flex-1 min-w-0 w-full'>
					...
					<div className='flex items-center gap-2'>
						<span className='relative w-4 h-4 shrink-0'>
							<Image
								src={platformImg(
									personal_info.platform_tool_name,
								)}
					...
					<div className='flex flex-wrap gap-4'>
						...
						<div>
							...
							<div className='flex items-center gap-1.5'>
								...
								<span className='font-bold'>
									{league.league_point.toLocaleString(
										"pt-BR",
									)}{" "}
									LP
								</span>
						...
						<div className='flex-1 min-w-[160px]'>
							...
							<div className='flex gap-2 mt-0.5 text-[10px] text-base-content/50'>
								<span>
									FG{" "}
									{playPoint.fighting_ground.toLocaleString(
										"pt-BR",
									)}
								</span>
								{playPoint.world_tour > 0 && (
									<span>
										WT{" "}
										{playPoint.world_tour.toLocaleString(
											"pt-BR",
										)}
									</span>
								)}
								<span>
									BH{" "}
									{playPoint.battle_hub.toLocaleString(
										"pt-BR",
									)}
								</span>
							</div>
					...
					<div className='flex items-center gap-2'>
						<div className='relative h-5 w-36'>
							<Image
								src={titleImg(
									info.title_data.title_data_plate_name,
								)}
						...
						<span className='text-xs text-base-content/60'>
							{info.title_data.title_data_val}
						</span>
					</div>
				</div>
			</div>
