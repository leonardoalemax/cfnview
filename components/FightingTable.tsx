...
export default function FightingTable({
	months,
	initialData,
	month,
	type,
	...
	function navigate(nextMonth: string, nextType: string, nextLeague: string) {
		...
		router.push(`/dados/matchups/${nextMonth}/${nextType}/${l}`, {
			scroll: false,
		});
	}
	...
	const opponents = inputFilter
		? allOpponents.filter((h) => h.input_type === inputFilter)
		: allOpponents;
	const records: FightingRecord[] = inputFilter
		? allRecords.filter((r) => r.input_type === inputFilter)
		: allRecords;

	const fullHeaderIndices = opponents.map((op) =>
		allOpponents.findIndex(
			(h) => h.id === op.id && h.input_type === op.input_type,
		),
	);

	const activeLeagueAlpha = activeLeague
		? String(activeLeague.league_rank)
		: "";
				...
				{!loading && records.length > 0 && (
					<div
						...
						<table className='table table-xs text-xs min-w-max'>
							<thead>
								<tr>
									<th className='sticky left-0 z-10 bg-base-200 min-w-[120px]'>
										Personagem
									</th>
								...
								{records
									...
									.map((rec, ri) => (
										<tr key={`${rec.id}-${rec.input_type}`}>
											<td
												className='sticky left-0 z-10 font-medium whitespace-nowrap bg-base-100'
												style={{
													backgroundColor:
														hovered?.row === ri
															? HIGHLIGHT
